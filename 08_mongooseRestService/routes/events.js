...
// Then there is also module 'event' which handles all CRUD operations
// for the data
let events = []

// URL osoite lisätään /api URL jolloin kokonaisuudessa tulee /api/events
router.route('/events')
// get All events
.get( (req, res) => {

    Event.find({}, (err, list) => {
     if (err) throw err // tai return next(err), kolmas parametri next()

      // object of all the users
      console.log('List of events:')
      console.log(list)
      //return res.json({message: 'List of events:'})
...
.post( (req, res) => {
    ...
    if (!req.body.hasOwnProperty('name')) {
        res.nameCode = 400 // res.status(400) tai res.statusCode = 400
